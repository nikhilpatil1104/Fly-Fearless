...
export function saveSearchToHistory(entry: Omit<SearchHistoryEntry, "searchedAt">) {
  try {
    ...
    // Deduplicate: remove previous identical route
    const deduped = existing.filter(
      (e) => !(e.origin === entry.origin && e.destination === entry.destination && e.tripType === entry.tripType)
    );
    ...
    // localStorage unavailable (SSR or private mode) — silent fail
  }
}
        ...
        <div className="flex items-center gap-2 mb-6">
          {(["All deals", "One way", "Roundtrip"] as const).map((f) => (
            <button
              ...
              type="button"
              onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all cursor-pointer select-none ${
                filter === f
