...
const AIRLINE_COLORS: Record<string, string> = {
  DL: "#E01933", AA: "#0078D2", UA: "#002244",
  WN: "#F9B612", B6: "#003876", AS: "#01426A",
  NK: "#3D0095", F9: "#006F53",
};
...
function DealsPageInner() {
  ...
  const currentDeal = FEATURED_DEALS.find(
    (d) => d.origIATA === origin && d.destIATA === destination
  ) ?? FEATURED_DEALS[0];
  ...
  const filteredDeals = FEATURED_DEALS.filter((d) =>
    filter === "All deals" ? true
    : filter === "One way"  ? d.type === "One-way"
    : d.type === "Roundtrip"
  );
    ...
    <div className="min-h-screen bg-[var(--sky-bg)]">
      ...
      <div className="max-w-[1400px] mx-auto px-4 lg:px-6 py-10">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
          Prices were available within the past 7 days and start at ${currentDeal.price} for
          one-way flights and ${currentDeal.price * 2 - 3} for round trip. Prices and
          availability are subject to change. Additional terms apply.
        </p>
          ...
          {filteredDeals.map((deal, i) => (
            ...
            >
              <div
                ...
                style={{ backgroundColor: AIRLINE_COLORS[deal.code] ?? "#374151" }}
              >
                {deal.code}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
          {ALT_AIRPORTS.map((a) => (
            <div
              key={a.iata}
              className="bg-white dark:bg-[#1C1F26] border border-gray-200 dark:border-gray-800
                         rounded-2xl p-5 hover:shadow-md cursor-pointer
                         hover:border-[var(--sky-primary)] transition-all"
            >
              <div className="flex justify-between items-center mb-1">
                <span className="font-bold text-gray-900 dark:text-white">{a.iata}</span>
                <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                  a.label === "Cheapest"
                    ? "bg-green-100 text-[var(--sky-green)]"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
          {[
            ...
            <div key={i} className={`${c.bg} rounded-2xl p-5 flex items-start gap-4`}>
              <div className={`${c.clr} w-10 h-10 rounded-full bg-white dark:bg-[#1C1F26] flex items-center justify-center shadow-sm`}>
                {c.icon}
              </div>
